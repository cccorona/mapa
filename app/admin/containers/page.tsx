...
export default function AdminContainersPage() {
  ...
  const startEdit = (c: LocationContainer) => {
    setEditingId(c.id)
    setEditForm({
      lat: String(c.lat),
      lng: String(c.lng),
  ...
  return (
    <main className="min-h-screen" style={{ background: "var(--background)" }}>
      <div className="max-w-2xl mx-auto px-6 py-12">
        ...
        <form onSubmit={handleCreate} className="border border-[var(--panel-border)] rounded-lg p-4 mb-6" style={{ background: "var(--panel-bg)" }}>
        ...
        {loading ? (
          ...
          <p className="font-mono text-sm text-[var(--parchment-dim)]">No hay puntos contenedores. Crea uno arriba.</p>
        ) : (
          <ul className="space-y-2">
            {containers.map((c) => (
              <li
                key={c.id}
                className="border border-[var(--panel-border)] rounded-lg p-3 flex flex-col gap-2"
