-- Tabla landmarks si no existe (por si 006 no se aplicó)
...
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'landmarks' AND policyname = 'landmarks_select') THEN
    CREATE POLICY "landmarks_select" ON landmarks FOR SELECT TO anon, authenticated USING (true);
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'landmarks' AND policyname = 'landmarks_insert') THEN
    CREATE POLICY "landmarks_insert" ON landmarks FOR INSERT TO authenticated WITH CHECK (true);
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'landmarks' AND policyname = 'landmarks_update') THEN
    CREATE POLICY "landmarks_update" ON landmarks FOR UPDATE TO authenticated USING (true) WITH CHECK (true);
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'landmarks' AND policyname = 'landmarks_delete') THEN
...
-- RPC insert_landmark (orden de parámetros: p_name, p_lng, p_lat, p_icon_url)
